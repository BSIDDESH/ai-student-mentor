// Skeleton primitives + full per-tab skeletons
// Every skeleton matches the exact layout of its real counterpart so
// the transition from skeleton → content feels instant, not janky.
...
function PulseDark({ className = "" }: { className?: string }) {
  return (
    <div
...
// ── Dashboard skeleton — matches DayArcHero + WeakTopicFlags + SubjectProgress ──

export function SkeletonDashboard() {
  return (
    <div
      className="min-h-screen flex flex-col md:flex-row"
      style={{ background: "var(--chalk, #faf9f7)" }}
      role="status"
      aria-label="Loading dashboard…"
    >
      {/* Sidebar stub — desktop only */}
      <aside className="hidden md:flex flex-col w-64 min-h-screen shrink-0 bg-white border-r border-stone-100">
        ...
        <div className="px-4 py-4 border-b border-stone-100">
          <div className="rounded-2xl p-4" style={{ background: "var(--mist, #f0ede8)" }}>
      ...
      {/* Bottom tab bar stub — mobile only */}
...
export function SkeletonHero() {
  return (
    <div className="rounded-2xl bg-indigo-600 p-6 overflow-hidden" aria-hidden>
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 space-y-3">
          <PulseDark className="h-4 w-32" />
          <PulseDark className="h-10 w-48" />
          <PulseDark className="h-4 w-56" />
          <div className="flex gap-3 mt-3">
            <PulseDark className="h-7 w-20 rounded-full" />
            <PulseDark className="h-7 w-28 rounded-full" />
          </div>
        </div>
        <div className="bg-white/20 rounded-2xl p-4 flex flex-col items-center gap-2 shrink-0">
          <PulseDark className="w-8 h-8 rounded-lg" />
          <PulseDark className="h-7 w-6" />
          <PulseDark className="h-3 w-10" />
        </div>
      </div>
      <div className="mt-6">
        <div className="flex justify-between mb-2">
          <PulseDark className="h-8 w-24" />
          <PulseDark className="h-4 w-28" />
        </div>
        <PulseDark className="h-2.5 w-full rounded-full" />
      </div>
    </div>
  );
}

// ── Focus-areas card skeleton (WeakTopicFlags) ─────────────────────────────────
...
function SkeletonFocusCard() {
  return (
    <div
      ...
      aria-hidden
    >
      <div className="h-1.5 bg-rose-300 w-full" />
      <div className="px-6 py-5 space-y-3">
        <div className="flex justify-between mb-4">
          <Pulse className="h-5 w-32 bg-rose-100" />
          <Pulse className="h-4 w-24 bg-rose-100" />
        </div>
        {[1, 2].map((i) => (
          <div key={i} className="flex items-center justify-between bg-white rounded-xl px-4 py-4 border border-rose-100">
            <div className="flex items-center gap-4">
              <Pulse className="w-14 h-14 rounded-xl bg-rose-100" />
              <div className="space-y-2">
                <Pulse className="h-4 w-24 bg-rose-100" />
                <Pulse className="h-3 w-16 bg-rose-100" />
              </div>
            </div>
            <Pulse className="h-9 w-24 rounded-full bg-rose-100" />
          </div>
        ))}
      </div>
    </div>
...
// ── Subject card skeleton (SubjectProgress) ────────────────────────────────────

function SkeletonSubjectCard() {
  return (
    <div className="bg-white rounded-2xl border border-stone-100 overflow-hidden" aria-hidden>
      <div className="flex items-center justify-between px-6 py-4 border-b border-stone-100" style={{ background: "var(--mist, #f0ede8)" }}>
