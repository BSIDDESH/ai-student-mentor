...
import { Droplets, Clock, Activity, Check, Sparkles, X } from "lucide-react";
...
export default function WellnessPanel({ profile, setProfile }: Props) {
  // Study session timer (seconds elapsed)
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSeconds((prev) => {
        const next = prev + 1;
        // Non-blocking nudge after 25 mins (1500 seconds)
        if (next === 1500) {
          setShowNudge(true);
        }
        return next;
  ...
  async function handleLog(type: "hydration" | "break" | "activity", actionKey: string) {
    ...
    try {
      ...
      const msg =
        type === "hydration"
          ? `Glass logged! +${res.xpEarned} XP 💧`
          : type === "break"
          ? `Break taken! +${res.xpEarned} XP ☕`
          : `Activity done! +${res.xpEarned} XP ✅`;
      showSuccess(msg);
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Couldn't log wellness activity. Please try again.";
      showError(msg);
    } finally {
